...
## Remote SQL, accounts, and traces

Migration 007 was applied in disposable PostgreSQL instances. Five account tests used actual reader, writer, and trace-service logins to verify private, tenant, and public memory visibility, revision authorship, immutable history, denied identity spoofing, trace privacy, and account revocation. Existing local task and catalog checks still passed.

Three forward tests copied the complete sql-context skill into a temporary installation and used its Python client against a real TLS server and PostgreSQL. They exercised trusted and untrusted certificates, rejected HTTP and redirects, bearer authentication, positional and named parameters, private memory writes, read-only rejection, cross-account prompt denial, and exact reconstruction of captured evidence from a prompt's trace. The independent test exposed and corrected module-launch and INSERT VALUES parser errors before publication.

The service suite passed 15 tests, including parser and hard-deadline tests plus real database checks. It verified 50-row and 64-KiB response bounds, large cells, special floating-point values, query cancellation, failed-attempt persistence, and a successful write whose trace-completion update failed. That last test preserved the committed memory and reported the incomplete trace rather than claiming rollback. Nested SQL that changed settings still hit PostgreSQL's armed statement timeout; cancellation also uses immutable backend coordinates.

The full check script ran 118 discovery tests successfully, with eight service integration tests deferred to the migrated disposable cluster, followed by 13 local runtime tests and the final 15-test service suite. The later service suite includes two additional regression tests. Launcher checks and skill validation passed. The built wheel contained the exact HTTPS client, SQL references, and migration bytes without Python cache files. Test servers and clusters were stopped and removed. No public HTTPS deployment or model-quality benchmark was performed.
