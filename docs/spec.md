...
The HTTPS SQL endpoint MUST accept one parsed data statement, reject transaction control and stacked statements, and account for data-modifying CTEs and functions with side effects. Reader execution MUST use a read-only transaction; writer execution MAY modify only memory allowed by its database grants and policies. DDL, role changes, COPY, and administrative commands belong outside the agent interface. A string-prefix test for `SELECT` is insufficient. A narrower reference reader MAY expose parameterized retrieval operations whose SQL remains inspectable. It must document that it does not accept unrestricted SQL.
...
The repository implements a local PostgreSQL service, fixed local SQL operations, an authenticated HTTPS SQL service, and a bootstrap agent skill. The local runtime covers a subset of the proposed service contract. This table distinguishes it from the remaining production work.
...
| Working-memory access | Forced RLS uses `session_user`. Remote execution uses account-specific reader/writer connections and private, tenant, or public memory visibility. | Public signup and federated identity are not provided. |
| Bounded SQL runner | The local helper exposes fixed operations; the HTTPS service accepts parsed data SQL within account permissions. Both bound output and use query deadlines. | Per-task call budgets and model-token accounting remain future work. |
| Local Package Skill | `package-context-sql-blue` uses the SHA-pinned Blue SDK, a copied launcher, and `colors.yml` to initialize local PostgreSQL and install the agent helper. The wheel includes the migrations and verified seed. Build and dry-run checks do not need a database. | The host needs the runtime toolchain, available through the supplied optional devenv files. Saved task transfer and automatic backup restore are separate work. |
| Local instance | `scripts/local_db.py` initializes persistent PostgreSQL outside Git, applies migrations and seed data, configures SCRAM over a private Unix socket, and provides lifecycle and backup commands. Reader and writer logins are distinct. | The HTTPS service provides bearer authentication for operator-provisioned accounts. Off-machine backups, monitoring, public signup, and federated identity remain deployment work. |
...
| Query tracing | The HTTPS service stores explicit prompts, SQL, parameters, outcomes, and optional bounded responses under account isolation. SQL comments connect execution to prompt and query UUIDs. | Conversation capture requires client submission. Cross-transaction commit uncertainty remains explicit. |
...
## 11. Remote accounts, memories, and traces

HTTPS requests MUST authenticate with a bearer credential. Trace IDs MUST NOT authorize access. The service MUST derive the account from its private credential mapping and connect directly as that account's selected reader or writer PostgreSQL login. Caller-provided identity settings MUST NOT determine row visibility. Each request MUST use a fresh connection; owner and trace-service credentials MUST NOT execute submitted SQL.

The remote catalog is shared, verified source material. `memory.entry` and `memory.revision` contain unverified account contributions. Entries default to private and MAY be explicitly tenant-shared or public to other authenticated accounts. Tenant publication MUST require membership. Only the author MAY append revisions. Entry visibility and earlier revisions are immutable; an account cannot overwrite another author's contribution. Remote memory is separate from the existing local `working` tables and does not silently publish local notes.

A client MUST explicitly submit prompt text or identify it as a summary. The service MUST store prompts separately from SQL comments. Each SQL attempt MUST reference a prompt owned by the authenticated account and receive a server-generated query UUID. A comment MUST link execution to those UUIDs. Submitted SQL and JSON parameters MUST remain recoverable. Prompt and query records MUST remain private to their account even when an associated memory is published.

The service MUST commit a query-attempt record before executing submitted SQL. Success and failure records MUST use a separate trace connection so rollback does not erase the attempt. Optional result capture MUST store the bounded response returned to the caller. Capturing prompts or results does not imply capturing the full conversation or all database rows. Authentication credentials MUST NOT be recorded as trace metadata or placed in comments. Clients MUST avoid submitting secrets as prompt text or SQL parameters.

SQL execution and trace completion are separate transactions. An interrupted worker or failed outcome write can leave execution uncertain. The service MUST report that uncertainty and MUST NOT retry writes automatically. A trace left in `started` after a process failure is incomplete evidence, not proof that SQL failed.

Trace-based model improvement is a review workflow. The reviewer SHOULD identify repeated retrieval difficulties, cite prompt/query IDs, and propose query, view, index, or schema changes. Candidate migrations MUST run in an isolated database and be evaluated on held-out requests, provenance preservation, and cross-account access tests. Fewer queries alone MUST NOT count as better answers. The agent SQL interface MUST NOT apply production DDL. See the installed improvement reference for the review prompt and evaluation record.
